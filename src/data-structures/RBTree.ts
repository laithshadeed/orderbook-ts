// Source: https://raw.githubusercontent.com/mikolalysenko/functional-red-black-tree/master/rbtree.ts
...
export class RedBlackTree<K, V> {
  constructor(public _compare: (a: K, b: K) => number, public root: RBNode<K, V> | undefined) {}

  get keys() {
    const result: Array<K> = [];
    this.forEach(function (k, _v) {
      result.push(k);
    });
    return result;
  }
...
class RedBlackTreeIterator<K, V> {
  constructor(public tree: RedBlackTree<K, V>, public _stack: Array<RBNode<K, V>>) {}
...
//Build a tree
export function createRBTree<K, V>(compare: (a: K, b: K) => number) {
